...
import com.vetan.service.RemCtService;
...
@Controller
@RequestMapping("/remct")
public class RemCtMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemCtService service;

	/**
	 * This method is used to fetch all remct entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemCts(Model model)
	{
		List<RemCtEntity> list = service.getAllRemCts();

		model.addAttribute("remcts", list);
		model.addAttribute("editRemCt", new RemCtEntity());
		return "remct-list";
	}

	/**
	 * This method is used to delete remct entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remct id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemCtById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemCtById(id);
		return "redirect:/remct";
	}
	 ...
	 * @param remct This is the remct entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemCt", method = RequestMethod.POST)
	public String createOrUpdateRemCt(RemCtEntity remct)
	{
		service.createOrUpdateRemCt(remct);
		return "redirect:/remct";
	}
