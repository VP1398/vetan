...
import com.vetan.service.LvMiscarriageService;
...
@Controller
@RequestMapping("/lvmiscarriage")
public class LvMiscarriageMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvMiscarriageService service;

	/**
	 * This method is used to fetch all lvmiscarriage entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvMiscarriages(Model model)
	{
		List<LvMiscarriageEntity> list = service.getAllLvMiscarriages();

		model.addAttribute("lvmiscarriages", list);
		model.addAttribute("editLvMiscarriage", new LvMiscarriageEntity());
		return "lvmiscarriage-list";
	}

	/**
	 * This method is used to delete lvmiscarriage entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvmiscarriage id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvMiscarriageById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvMiscarriageById(id);
		return "redirect:/lvmiscarriage";
	}
	 ...
	 * @param lvmiscarriage This is the lvmiscarriage entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvMiscarriage", method = RequestMethod.POST)
	public String createOrUpdateLvMiscarriage(LvMiscarriageEntity lvmiscarriage)
	{
		service.createOrUpdateLvMiscarriage(lvmiscarriage);
		return "redirect:/lvmiscarriage";
	}
