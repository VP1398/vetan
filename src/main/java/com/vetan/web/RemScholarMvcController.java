...
import com.vetan.service.RemScholarService;
...
@Controller
@RequestMapping("/remscholar")
public class RemScholarMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemScholarService service;

	/**
	 * This method is used to fetch all remscholar entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemScholars(Model model)
	{
		List<RemScholarEntity> list = service.getAllRemScholars();

		model.addAttribute("remscholars", list);
		model.addAttribute("editRemScholar", new RemScholarEntity());
		return "remscholar-list";
	}

	/**
	 * This method is used to delete remscholar entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remscholar id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemScholarById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemScholarById(id);
		return "redirect:/remscholar";
	}
	 ...
	 * @param remscholar This is the remscholar entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemScholar", method = RequestMethod.POST)
	public String createOrUpdateRemScholar(RemScholarEntity remscholar)
	{
		service.createOrUpdateRemScholar(remscholar);
		return "redirect:/remscholar";
	}
