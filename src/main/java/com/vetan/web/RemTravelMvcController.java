...
import com.vetan.service.RemTravelService;
...
@Controller
@RequestMapping("/remtravel")
public class RemTravelMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemTravelService service;

	/**
	 * This method is used to fetch all remtravel entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemTravels(Model model)
	{
		List<RemTravelEntity> list = service.getAllRemTravels();

		model.addAttribute("remtravels", list);
		model.addAttribute("editRemTravel", new RemTravelEntity());
		return "remtravel-list";
	}

	/**
	 * This method is used to delete remtravel entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remtravel id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemTravelById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemTravelById(id);
		return "redirect:/remtravel";
	}
	 ...
	 * @param remtravel This is the remtravel entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemTravel", method = RequestMethod.POST)
	public String createOrUpdateRemTravel(RemTravelEntity remtravel)
	{
		service.createOrUpdateRemTravel(remtravel);
		return "redirect:/remtravel";
	}
