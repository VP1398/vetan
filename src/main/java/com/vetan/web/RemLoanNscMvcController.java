...
import com.vetan.service.RemLoanNscService;
...
@Controller
@RequestMapping("/remloannsc")
public class RemLoanNscMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemLoanNscService service;

	/**
	 * This method is used to fetch all remloannsc entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemLoanNscs(Model model)
	{
		List<RemLoanNscEntity> list = service.getAllRemLoanNscs();

		model.addAttribute("remloannscs", list);
		model.addAttribute("editRemLoanNsc", new RemLoanNscEntity());
		return "remloannsc-list";
	}

	/**
	 * This method is used to delete remloannsc entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remloannsc id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemLoanNscById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemLoanNscById(id);
		return "redirect:/remloannsc";
	}
	 ...
	 * @param remloannsc This is the remloannsc entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemLoanNsc", method = RequestMethod.POST)
	public String createOrUpdateRemLoanNsc(RemLoanNscEntity remloannsc)
	{
		service.createOrUpdateRemLoanNsc(remloannsc);
		return "redirect:/remloannsc";
	}
