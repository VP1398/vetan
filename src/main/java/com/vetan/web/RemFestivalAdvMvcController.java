...
import com.vetan.service.RemFestivalAdvService;
...
@Controller
@RequestMapping("/remfestivaladv")
public class RemFestivalAdvMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemFestivalAdvService service;

	/**
	 * This method is used to fetch all remfestivaladv entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemFestivalAdvs(Model model)
	{
		List<RemFestivalAdvEntity> list = service.getAllRemFestivalAdvs();

		model.addAttribute("remfestivaladvs", list);
		model.addAttribute("editRemFestivalAdv", new RemFestivalAdvEntity());
		return "remfestivaladv-list";
	}

	/**
	 * This method is used to delete remfestivaladv entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remfestivaladv id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemFestivalAdvById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemFestivalAdvById(id);
		return "redirect:/remfestivaladv";
	}
	 ...
	 * @param remfestivaladv This is the remfestivaladv entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemFestivalAdv", method = RequestMethod.POST)
	public String createOrUpdateRemFestivalAdv(RemFestivalAdvEntity remfestivaladv)
	{
		service.createOrUpdateRemFestivalAdv(remfestivaladv);
		return "redirect:/remfestivaladv";
	}
