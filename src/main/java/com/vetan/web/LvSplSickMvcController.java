...
@Controller
@RequestMapping("/lvsplsick")
public class LvSplSickMvcController
{
	...
	/**
	 * This method is used to fetch all lvsplsick entries from the database and
	...
	public String getAllLvAdoptions(Model model)
	{
		...
		model.addAttribute("lvsplsicks", list);
		model.addAttribute("editLvAdoption", new LvAdoptionEntity());
		return "lvsplsick-list";
	}

	/**
	 * This method is used to delete lvsplsick entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvsplsick id which is the primary key
	 ...
	 * @param lvsplsick This is the lvsplsick entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvAdoption", method = RequestMethod.POST)
	public String createOrUpdateLvAdoption(LvAdoptionEntity lvsplsick)
	{
		service.createOrUpdateLvAdoption(lvsplsick);
		return "redirect:/lvsplsick";
	}
