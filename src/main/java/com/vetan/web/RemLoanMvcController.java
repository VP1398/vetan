...
import com.vetan.service.RemLoanService;
...
public class RemLoanMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemLoanService service;

	/**
	 * This method is used to fetch all remloan entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemLoans(Model model)
	{
		List<RemLoanEntity> list = service.getAllRemLoans();

		model.addAttribute("remloans", list);
		model.addAttribute("editRemLoan", new RemLoanEntity());
		return "remloan-list";
	}

	/**
	 * This method is used to delete remloan entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remloan id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemLoanById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemLoanById(id);
		return "redirect:/remloan";
	}
	 ...
	 * @param remloan This is the remloan entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemLoan", method = RequestMethod.POST)
	public String createOrUpdateRemLoan(RemLoanEntity remloan)
	{
		service.createOrUpdateRemLoan(remloan);
		return "redirect:/remloan";
	}
