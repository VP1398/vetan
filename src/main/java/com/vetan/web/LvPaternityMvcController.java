...
import com.vetan.service.LvPaternityService;
...
@Controller
@RequestMapping("/lvpaternity")
public class LvPaternityMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvPaternityService service;

	/**
	 * This method is used to fetch all lvpaternity entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvPaternitys(Model model)
	{
		List<LvPaternityEntity> list = service.getAllLvPaternitys();

		model.addAttribute("lvpaternitys", list);
		model.addAttribute("editLvPaternity", new LvPaternityEntity());
		return "lvpaternity-list";
	}

	/**
	 * This method is used to delete lvpaternity entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvpaternity id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvPaternityById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvPaternityById(id);
		return "redirect:/lvpaternity";
	}
	 ...
	 * @param lvpaternity This is the lvpaternity entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvPaternity", method = RequestMethod.POST)
	public String createOrUpdateLvPaternity(LvPaternityEntity lvpaternity)
	{
		service.createOrUpdateLvPaternity(lvpaternity);
		return "redirect:/lvpaternity";
	}
