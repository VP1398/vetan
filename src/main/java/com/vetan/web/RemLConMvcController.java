...
import com.vetan.service.RemLConService;
...
@Controller
@RequestMapping("/remlcon")
public class RemLConMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemLConService service;

	/**
	 * This method is used to fetch all remlcon entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemLCons(Model model)
	{
		List<RemLConEntity> list = service.getAllRemLCons();

		model.addAttribute("remlcons", list);
		model.addAttribute("editRemLCon", new RemLConEntity());
		return "remlcon-list";
	}

	/**
	 * This method is used to delete remlcon entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remlcon id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemLConById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemLConById(id);
		return "redirect:/remlcon";
	}
	 ...
	 * @param remlcon This is the remlcon entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemLCon", method = RequestMethod.POST)
	public String createOrUpdateRemLCon(RemLConEntity remlcon)
	{
		service.createOrUpdateRemLCon(remlcon);
		return "redirect:/remlcon";
	}
