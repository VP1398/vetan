...
import com.vetan.service.LvSickService;
...
@Controller
@RequestMapping("/lvsick")
public class LvSickMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvSickService service;

	/**
	 * This method is used to fetch all lvsick entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvSicks(Model model)
	{
		List<LvSickEntity> list = service.getAllLvSicks();

		model.addAttribute("lvsicks", list);
		model.addAttribute("editLvSick", new LvSickEntity());
		return "lvsick-list";
	}

	/**
	 * This method is used to delete lvsick entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvsick id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvSickById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvSickById(id);
		return "redirect:/lvsick";
	}
	 ...
	 * @param lvsick This is the lvsick entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvSick", method = RequestMethod.POST)
	public String createOrUpdateLvSick(LvSickEntity lvsick)
	{
		service.createOrUpdateLvSick(lvsick);
		return "redirect:/lvsick";
	}
