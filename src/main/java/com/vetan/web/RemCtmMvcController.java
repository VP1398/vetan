...
import com.vetan.service.RemCtmService;
...
@Controller
@RequestMapping("/remctm")
public class RemCtmMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemCtmService service;

	/**
	 * This method is used to fetch all remctm entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemCtms(Model model)
	{
		List<RemCtmEntity> list = service.getAllRemCtms();

		model.addAttribute("remctms", list);
		model.addAttribute("editRemCtm", new RemCtmEntity());
		return "remctm-list";
	}

	/**
	 * This method is used to delete remctm entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remctm id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemCtmById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemCtmById(id);
		return "redirect:/remctm";
	}
	 ...
	 * @param remctm This is the remctm entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemCtm", method = RequestMethod.POST)
	public String createOrUpdateRemCtm(RemCtmEntity remctm)
	{
		service.createOrUpdateRemCtm(remctm);
		return "redirect:/remctm";
	}
