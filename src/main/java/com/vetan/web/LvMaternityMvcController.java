...
import com.vetan.service.LvMaternityService;
...
@Controller
@RequestMapping("/lvmaternity")
public class LvMaternityMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvMaternityService service;

	/**
	 * This method is used to fetch all lvmaternity entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvMaternitys(Model model)
	{
		List<LvMaternityEntity> list = service.getAllLvMaternitys();

		model.addAttribute("lvmaternitys", list);
		model.addAttribute("editLvMaternity", new LvMaternityEntity());
		return "lvmaternity-list";
	}

	/**
	 * This method is used to delete lvmaternity entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvmaternity id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvMaternityById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvMaternityById(id);
		return "redirect:/lvmaternity";
	}
	 ...
	 * @param lvmaternity This is the lvmaternity entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvMaternity", method = RequestMethod.POST)
	public String createOrUpdateLvMaternity(LvMaternityEntity lvmaternity)
	{
		service.createOrUpdateLvMaternity(lvmaternity);
		return "redirect:/lvmaternity";
	}
