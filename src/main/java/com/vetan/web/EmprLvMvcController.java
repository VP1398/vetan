...
import com.vetan.service.EmprLvService;
...
@Controller
@RequestMapping("/emprlv")
public class EmprLvMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmprLvService service;

	/**
	 * This method is used to fetch all emprlv entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllEmprLvs(Model model)
	{
		List<EmprLvEntity> list = service.getAllEmprLvs();

		model.addAttribute("emprlvs", list);
		model.addAttribute("editEmprLv", new EmprLvEntity());
		return "emprlv-list";
	}

	/**
	 * This method is used to delete emprlv entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the emprlv id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteEmprLvById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteEmprLvById(id);
		return "redirect:/emprlv";
	}
	 ...
	 * @param emprlv This is the emprlv entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createEmprLv", method = RequestMethod.POST)
	public String createOrUpdateEmprLv(EmprLvEntity emprlv)
	{
		service.createOrUpdateEmprLv(emprlv);
		return "redirect:/emprlv";
	}
