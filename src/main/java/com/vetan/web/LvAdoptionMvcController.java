...
import com.vetan.service.LvAdoptionService;
...
@Controller
@RequestMapping("/lvadoption")
public class LvAdoptionMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvAdoptionService service;

	/**
	 * This method is used to fetch all lvadoption entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvAdoptions(Model model)
	{
		List<LvAdoptionEntity> list = service.getAllLvAdoptions();

		model.addAttribute("lvadoptions", list);
		model.addAttribute("editLvAdoption", new LvAdoptionEntity());
		return "lvadoption-list";
	}

	/**
	 * This method is used to delete lvadoption entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvadoption id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvAdoptionById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvAdoptionById(id);
		return "redirect:/lvadoption";
	}
	 ...
	 * @param lvadoption This is the lvadoption entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvAdoption", method = RequestMethod.POST)
	public String createOrUpdateLvAdoption(LvAdoptionEntity lvadoption)
	{
		service.createOrUpdateLvAdoption(lvadoption);
		return "redirect:/lvadoption";
	}
