...
import com.vetan.service.RemPEService;
...
@Controller
@RequestMapping("/rempe")
public class RemPEMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemPEService service;

	/**
	 * This method is used to fetch all rempe entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemPEs(Model model)
	{
		List<RemPEEntity> list = service.getAllRemPEs();

		model.addAttribute("rempes", list);
		model.addAttribute("editRemPE", new RemPEEntity());
		return "rempe-list";
	}

	/**
	 * This method is used to delete rempe entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the rempe id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemPEById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemPEById(id);
		return "redirect:/rempe";
	}
	 ...
	 * @param rempe This is the rempe entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemPE", method = RequestMethod.POST)
	public String createOrUpdateRemPE(RemPEEntity rempe)
	{
		service.createOrUpdateRemPE(rempe);
		return "redirect:/rempe";
	}
