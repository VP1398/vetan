...
import com.vetan.service.LvCasualService;
...
@Controller
@RequestMapping("/lvcasual")
public class LvCasualMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvCasualService service;

	/**
	 * This method is used to fetch all lvcasual entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvCasuals(Model model)
	{
		List<LvCasualEntity> list = service.getAllLvCasuals();

		model.addAttribute("lvcasuals", list);
		model.addAttribute("editLvCasual", new LvCasualEntity());
		return "lvcasual-list";
	}

	/**
	 * This method is used to delete lvcasual entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvcasual id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvCasualById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvCasualById(id);
		return "redirect:/lvcasual";
	}
	 ...
	 * @param lvcasual This is the lvcasual entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvCasual", method = RequestMethod.POST)
	public String createOrUpdateLvCasual(LvCasualEntity lvcasual)
	{
		service.createOrUpdateLvCasual(lvcasual);
		return "redirect:/lvcasual";
	}
