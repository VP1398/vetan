...
import com.vetan.service.RemSalAdvService;
...
@Controller
@RequestMapping("/remsaladv")
public class RemSalAdvMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	RemSalAdvService service;

	/**
	 * This method is used to fetch all remsaladv entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllRemSalAdvs(Model model)
	{
		List<RemSalAdvEntity> list = service.getAllRemSalAdvs();

		model.addAttribute("remsaladvs", list);
		model.addAttribute("editRemSalAdv", new RemSalAdvEntity());
		return "remsaladv-list";
	}

	/**
	 * This method is used to delete remsaladv entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the remsaladv id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteRemSalAdvById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteRemSalAdvById(id);
		return "redirect:/remsaladv";
	}
	 ...
	 * @param remsaladv This is the remsaladv entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createRemSalAdv", method = RequestMethod.POST)
	public String createOrUpdateRemSalAdv(RemSalAdvEntity remsaladv)
	{
		service.createOrUpdateRemSalAdv(remsaladv);
		return "redirect:/remsaladv";
	}
