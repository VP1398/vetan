...
import com.vetan.service.LvPrivilegeService;
...
@Controller
@RequestMapping("/lvprivilege")
public class LvPrivilegeMvcController
{
	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvPrivilegeService service;

	/**
	 * This method is used to fetch all lvprivilege entries from the database and
	 ...
	 */
	@RequestMapping
	public String getAllLvPrivileges(Model model)
	{
		List<LvPrivilegeEntity> list = service.getAllLvPrivileges();

		model.addAttribute("lvprivileges", list);
		model.addAttribute("editLvPrivilege", new LvPrivilegeEntity());
		return "lvprivilege-list";
	}

	/**
	 * This method is used to delete lvprivilege entry from the database by its id.
	 * @param model This attribute is used to set values which will be used in the html template to populate data.
	 * @param id This is the lvprivilege id which is the primary key
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/delete/{id}")
	public String deleteLvPrivilegeById(Model model, @PathVariable("id") Long id)
							throws RecordNotFoundException 
	{
		service.deleteLvPrivilegeById(id);
		return "redirect:/lvprivilege";
	}
	 ...
	 * @param lvprivilege This is the lvprivilege entity with all its value pre-populated.
	 * @return String This returns the name of the html page it has to be redirected.
	 */
	@RequestMapping(path = "/createLvPrivilege", method = RequestMethod.POST)
	public String createOrUpdateLvPrivilege(LvPrivilegeEntity lvprivilege)
	{
		service.createOrUpdateLvPrivilege(lvprivilege);
		return "redirect:/lvprivilege";
	}
