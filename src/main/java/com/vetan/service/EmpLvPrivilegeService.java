...
import com.vetan.repository.EmpLvPrivilegeRepository;
...
@Service
public class EmpLvPrivilegeService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvPrivilegeRepository repository;

	/**
	 * This method is used to fetch all the emplvprivilege entries from the database.
	 * @return List<EmpLvPrivilegeEntity> This returns the list of emplvprivilege entities fetched from the database.
	 */
	public List<EmpLvPrivilegeEntity> getAllEmpLvPrivileges()
	{
		List<EmpLvPrivilegeEntity> result = (List<EmpLvPrivilegeEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvPrivilegeEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvPrivilegeEntity>();
		}
	}
	 ...
	 * @return EmpLvPrivilegeEntity This returns the EmpLvPrivilegeEntity object for the requested emplvadoption id.
	 */
	public EmpLvPrivilegeEntity getEmpLvPrivilegeById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvPrivilegeEntity> emplvadoption = repository.findById(id);
	...
	/**
	 * This method is used to update all the columns of the EmpLvPrivilege entry in the database.
	 * @param entity This attribute is the EmpLvPrivilegeEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvPrivilegeEntity object.
	 */
	public EmpLvPrivilegeEntity createOrUpdateEmpLvPrivilege(EmpLvPrivilegeEntity entity)
	{
		...
		{
			Optional<EmpLvPrivilegeEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvPrivilegeEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvprivilege entity.
	 */
	...
	/**
	 * This method is used to delete the emplvprivilege entry from the database by its id.
	 * @param id This attribute is the id of the emplvprivilege entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvPrivilegeById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvPrivilegeEntity> employee = repository.findById(id);
		
		if(employee.isPresent()) 
		{
			repository.deleteById(id);
		} else {
			throw new RecordNotFoundException("No emplvprivilege record exist for given id");
		}
	} 
}
