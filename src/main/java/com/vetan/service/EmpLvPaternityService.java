...
import com.vetan.repository.EmpLvPaternityRepository;
...
@Service
public class EmpLvPaternityService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvPaternityRepository repository;

	/**
	 * This method is used to fetch all the emplvpaternity entries from the database.
	 * @return List<EmpLvPaternityEntity> This returns the list of emplvpaternity entities fetched from the database.
	 */
	public List<EmpLvPaternityEntity> getAllEmpLvPaternitys()
	{
		List<EmpLvPaternityEntity> result = (List<EmpLvPaternityEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvPaternityEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvPaternityEntity>();
		}
	}
	 ...
	 * @return EmpLvPaternityEntity This returns the EmpLvPaternityEntity object for the requested emplvadoption id.
	 */
	public EmpLvPaternityEntity getEmpLvPaternityById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvPaternityEntity> emplvadoption = repository.findById(id);
	...
	/**
	 * This method is used to update all the columns of the EmpLvPaternity entry in the database.
	 * @param entity This attribute is the EmpLvPaternityEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvPaternityEntity object.
	 */
	public EmpLvPaternityEntity createOrUpdateEmpLvPaternity(EmpLvPaternityEntity entity)
	{
		...
		{
			Optional<EmpLvPaternityEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvPaternityEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvpaternity entity.
	 */
	...
	/**
	 * This method is used to delete the emplvpaternity entry from the database by its id.
	 * @param id This attribute is the id of the emplvpaternity entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvPaternityById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvPaternityEntity> employee = repository.findById(id);
		
		if(employee.isPresent()) 
		{
			repository.deleteById(id);
		} else {
			throw new RecordNotFoundException("No emplvpaternity record exist for given id");
		}
	} 
}
