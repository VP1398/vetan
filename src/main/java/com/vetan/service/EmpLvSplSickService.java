...
import com.vetan.repository.EmpLvSplSickRepository;
...
@Service
public class EmpLvSplSickService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvSplSickRepository repository;

	/**
	 * This method is used to fetch all the emplvsplsick entries from the database.
	 * @return List<EmpLvSplSickEntity> This returns the list of emplvsplsick entities fetched from the database.
	 */
	public List<EmpLvSplSickEntity> getAllEmpLvSplSicks()
	{
		List<EmpLvSplSickEntity> result = (List<EmpLvSplSickEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvSplSickEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvSplSickEntity>();
		}
	}
	 ...
	 * @return EmpLvSplSickEntity This returns the EmpLvSplSickEntity object for the requested emplvadoption id.
	 */
	public EmpLvSplSickEntity getEmpLvSplSickById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvSplSickEntity> emplvadoption = repository.findById(id);
	...
	/**
	 * This method is used to update all the columns of the EmpLvSplSick entry in the database.
	 * @param entity This attribute is the EmpLvSplSickEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvSplSickEntity object.
	 */
	public EmpLvSplSickEntity createOrUpdateEmpLvSplSick(EmpLvSplSickEntity entity)
	{
		...
		{
			Optional<EmpLvSplSickEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvSplSickEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvsplsick entity.
	 */
	...
	/**
	 * This method is used to delete the emplvsplsick entry from the database by its id.
	 * @param id This attribute is the id of the emplvsplsick entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvSplSickById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvSplSickEntity> employee = repository.findById(id);
		
		if(employee.isPresent()) 
		{
			repository.deleteById(id);
		} else {
			throw new RecordNotFoundException("No emplvsplsick record exist for given id");
		}
	} 
}
