...
import com.vetan.repository.LvSickRepository;
...
@Service
public class LvSickService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvSickRepository repository;

	/**
	 * This method is used to fetch all the lvsick entries from the database.
	 * @return List<LvSickEntity> This returns the list of lvsick entities fetched from the database.
	 */
	public List<LvSickEntity> getAllLvSicks()
	{
		List<LvSickEntity> result = (List<LvSickEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvSickEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvSickEntity>();
		}
	}

	/**
	 * This method is used to get a single lvsick entry from the database based on its id.
	 * @param id This attribute is the id of the lvsick entry.
	 * @return LvSickEntity This returns the LvSickEntity object for the requested lvsick id.
	 */
	public LvSickEntity getLvSickById(Long id) throws RecordNotFoundException
	{
		Optional<LvSickEntity> lvsick = repository.findById(id);
		
		if(lvsick.isPresent()) {
			return lvsick.get();
		} else {
			throw new RecordNotFoundException("No lvsick record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvSick entry in the database.
	 * @param entity This attribute is the LvSickEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvSickEntity object.
	 */
	public LvSickEntity createOrUpdateLvSick(LvSickEntity entity)
	{
		...
		{
			Optional<LvSickEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvSickEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvsick entity.
	 */
	...
	/**
	 * This method is used to delete the lvsick entry from the database by its id.
	 * @param id This attribute is the id of the lvsick entry to be deleted.
	 * @return void
	 */
	public void deleteLvSickById(Long id) throws RecordNotFoundException
	{
		Optional<LvSickEntity> employee = repository.findById(id);
