...
import com.vetan.repository.LvExtraOrdinaryRepository;
...
@Service
public class LvExtraOrdinaryService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvExtraOrdinaryRepository repository;

	/**
	 * This method is used to fetch all the lvextraordinary entries from the database.
	 * @return List<LvExtraOrdinaryEntity> This returns the list of lvextraordinary entities fetched from the database.
	 */
	public List<LvExtraOrdinaryEntity> getAllLvExtraOrdinarys()
	{
		List<LvExtraOrdinaryEntity> result = (List<LvExtraOrdinaryEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvExtraOrdinaryEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvExtraOrdinaryEntity>();
		}
	}

	/**
	 * This method is used to get a single lvextraordinary entry from the database based on its id.
	 * @param id This attribute is the id of the lvextraordinary entry.
	 * @return LvExtraOrdinaryEntity This returns the LvExtraOrdinaryEntity object for the requested lvextraordinary id.
	 */
	public LvExtraOrdinaryEntity getLvExtraOrdinaryById(Long id) throws RecordNotFoundException
	{
		Optional<LvExtraOrdinaryEntity> lvextraordinary = repository.findById(id);
		
		if(lvextraordinary.isPresent()) {
			return lvextraordinary.get();
		} else {
			throw new RecordNotFoundException("No lvextraordinary record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvExtraOrdinary entry in the database.
	 * @param entity This attribute is the LvExtraOrdinaryEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvExtraOrdinaryEntity object.
	 */
	public LvExtraOrdinaryEntity createOrUpdateLvExtraOrdinary(LvExtraOrdinaryEntity entity)
	{
		...
		{
			Optional<LvExtraOrdinaryEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvExtraOrdinaryEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvextraordinary entity.
	 */
	...
	/**
	 * This method is used to delete the lvextraordinary entry from the database by its id.
	 * @param id This attribute is the id of the lvextraordinary entry to be deleted.
	 * @return void
	 */
	public void deleteLvExtraOrdinaryById(Long id) throws RecordNotFoundException
	{
		Optional<LvExtraOrdinaryEntity> employee = repository.findById(id);
