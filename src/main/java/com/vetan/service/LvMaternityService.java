...
import com.vetan.repository.LvMaternityRepository;
...
@Service
public class LvMaternityService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvMaternityRepository repository;

	/**
	 * This method is used to fetch all the lvmaternity entries from the database.
	 * @return List<LvMaternityEntity> This returns the list of lvmaternity entities fetched from the database.
	 */
	public List<LvMaternityEntity> getAllLvMaternitys()
	{
		List<LvMaternityEntity> result = (List<LvMaternityEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvMaternityEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvMaternityEntity>();
		}
	}

	/**
	 * This method is used to get a single lvmaternity entry from the database based on its id.
	 * @param id This attribute is the id of the lvmaternity entry.
	 * @return LvMaternityEntity This returns the LvMaternityEntity object for the requested lvmaternity id.
	 */
	public LvMaternityEntity getLvMaternityById(Long id) throws RecordNotFoundException
	{
		Optional<LvMaternityEntity> lvmaternity = repository.findById(id);
		
		if(lvmaternity.isPresent()) {
			return lvmaternity.get();
		} else {
			throw new RecordNotFoundException("No lvmaternity record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvMaternity entry in the database.
	 * @param entity This attribute is the LvMaternityEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvMaternityEntity object.
	 */
	public LvMaternityEntity createOrUpdateLvMaternity(LvMaternityEntity entity)
	{
		...
		{
			Optional<LvMaternityEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvMaternityEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvmaternity entity.
	 */
	...
	/**
	 * This method is used to delete the lvmaternity entry from the database by its id.
	 * @param id This attribute is the id of the lvmaternity entry to be deleted.
	 * @return void
	 */
	public void deleteLvMaternityById(Long id) throws RecordNotFoundException
	{
		Optional<LvMaternityEntity> employee = repository.findById(id);
