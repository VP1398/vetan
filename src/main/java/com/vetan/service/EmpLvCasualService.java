...
import com.vetan.repository.EmpLvCasualRepository;
...
@Service
public class EmpLvCasualService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvCasualRepository repository;

	/**
	 * This method is used to fetch all the emplvcasual entries from the database.
	 * @return List<EmpLvCasualEntity> This returns the list of emplvcasual entities fetched from the database.
	 */
	public List<EmpLvCasualEntity> getAllEmpLvCasuals()
	{
		List<EmpLvCasualEntity> result = (List<EmpLvCasualEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvCasualEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvCasualEntity>();
		}
	}

	/**
	 * This method is used to get a single emplvcasual entry from the database based on its id.
	 * @param id This attribute is the id of the emplvcasual entry.
	 * @return EmpLvCasualEntity This returns the EmpLvCasualEntity object for the requested emplvcasual id.
	 */
	public EmpLvCasualEntity getEmpLvCasualById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvCasualEntity> emplvcasual = repository.findById(id);
		
		if(emplvcasual.isPresent()) {
			return emplvcasual.get();
		} else {
			throw new RecordNotFoundException("No emplvcasual record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the EmpLvCasual entry in the database.
	 * @param entity This attribute is the EmpLvCasualEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvCasualEntity object.
	 */
	public EmpLvCasualEntity createOrUpdateEmpLvCasual(EmpLvCasualEntity entity)
	{
		...
		{
			Optional<EmpLvCasualEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvCasualEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvcasual entity.
	 */
	...
	/**
	 * This method is used to delete the emplvcasual entry from the database by its id.
	 * @param id This attribute is the id of the emplvcasual entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvCasualById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvCasualEntity> employee = repository.findById(id);
