...
import com.vetan.repository.LvPaternityRepository;
...
@Service
public class LvPaternityService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvPaternityRepository repository;

	/**
	 * This method is used to fetch all the lvpaternity entries from the database.
	 * @return List<LvPaternityEntity> This returns the list of lvpaternity entities fetched from the database.
	 */
	public List<LvPaternityEntity> getAllLvPaternitys()
	{
		List<LvPaternityEntity> result = (List<LvPaternityEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvPaternityEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvPaternityEntity>();
		}
	}

	/**
	 * This method is used to get a single lvpaternity entry from the database based on its id.
	 * @param id This attribute is the id of the lvpaternity entry.
	 * @return LvPaternityEntity This returns the LvPaternityEntity object for the requested lvpaternity id.
	 */
	public LvPaternityEntity getLvPaternityById(Long id) throws RecordNotFoundException
	{
		Optional<LvPaternityEntity> lvpaternity = repository.findById(id);
		
		if(lvpaternity.isPresent()) {
			return lvpaternity.get();
		} else {
			throw new RecordNotFoundException("No lvpaternity record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvPaternity entry in the database.
	 * @param entity This attribute is the LvPaternityEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvPaternityEntity object.
	 */
	public LvPaternityEntity createOrUpdateLvPaternity(LvPaternityEntity entity)
	{
		...
		{
			Optional<LvPaternityEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvPaternityEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvpaternity entity.
	 */
	...
	/**
	 * This method is used to delete the lvpaternity entry from the database by its id.
	 * @param id This attribute is the id of the lvpaternity entry to be deleted.
	 * @return void
	 */
	public void deleteLvPaternityById(Long id) throws RecordNotFoundException
	{
		Optional<LvPaternityEntity> employee = repository.findById(id);
