...
import com.vetan.repository.EmpLvSickRepository;
...
@Service
public class EmpLvSickService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvSickRepository repository;

	/**
	 * This method is used to fetch all the emplvsick entries from the database.
	 * @return List<EmpLvSickEntity> This returns the list of emplvsick entities fetched from the database.
	 */
	public List<EmpLvSickEntity> getAllEmpLvSicks()
	{
		List<EmpLvSickEntity> result = (List<EmpLvSickEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvSickEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvSickEntity>();
		}
	}

	/**
	 * This method is used to get a single emplvsick entry from the database based on its id.
	 * @param id This attribute is the id of the emplvsick entry.
	 * @return EmpLvSickEntity This returns the EmpLvSickEntity object for the requested emplvsick id.
	 */
	public EmpLvSickEntity getEmpLvSickById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvSickEntity> emplvsick = repository.findById(id);
		
		if(emplvsick.isPresent()) {
			return emplvsick.get();
		} else {
			throw new RecordNotFoundException("No emplvsick record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the EmpLvSick entry in the database.
	 * @param entity This attribute is the EmpLvSickEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvSickEntity object.
	 */
	public EmpLvSickEntity createOrUpdateEmpLvSick(EmpLvSickEntity entity)
	{
		...
		{
			Optional<EmpLvSickEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvSickEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvsick entity.
	 */
	...
	/**
	 * This method is used to delete the emplvsick entry from the database by its id.
	 * @param id This attribute is the id of the emplvsick entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvSickById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvSickEntity> employee = repository.findById(id);
