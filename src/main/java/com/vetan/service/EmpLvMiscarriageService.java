...
import com.vetan.repository.EmpLvMiscarriageRepository;
...
@Service
public class EmpLvMiscarriageService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvMiscarriageRepository repository;

	/**
	 * This method is used to fetch all the emplvmiscarriage entries from the database.
	 * @return List<EmpLvMiscarriageEntity> This returns the list of emplvmiscarriage entities fetched from the database.
	 */
	public List<EmpLvMiscarriageEntity> getAllEmpLvMiscarriages()
	{
		List<EmpLvMiscarriageEntity> result = (List<EmpLvMiscarriageEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvMiscarriageEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvMiscarriageEntity>();
		}
	}
	 ...
	 * @return EmpLvMiscarriageEntity This returns the EmpLvMiscarriageEntity object for the requested emplvadoption id.
	 */
	public EmpLvMiscarriageEntity getEmpLvMiscarriageById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvMiscarriageEntity> emplvadoption = repository.findById(id);
	...
	/**
	 * This method is used to update all the columns of the EmpLvMiscarriage entry in the database.
	 * @param entity This attribute is the EmpLvMiscarriageEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvMiscarriageEntity object.
	 */
	public EmpLvMiscarriageEntity createOrUpdateEmpLvMiscarriage(EmpLvMiscarriageEntity entity)
	{
		...
		{
			Optional<EmpLvMiscarriageEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvMiscarriageEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvmiscarriage entity.
	 */
	...
	/**
	 * This method is used to delete the emplvmiscarriage entry from the database by its id.
	 * @param id This attribute is the id of the emplvmiscarriage entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvMiscarriageById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvMiscarriageEntity> employee = repository.findById(id);
		
		if(employee.isPresent()) 
		{
			repository.deleteById(id);
		} else {
			throw new RecordNotFoundException("No emplvmiscarriage record exist for given id");
		}
	} 
}
