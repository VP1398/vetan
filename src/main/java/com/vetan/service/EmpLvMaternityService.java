...
import com.vetan.repository.EmpLvMaternityRepository;
...
@Service
public class EmpLvMaternityService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	EmpLvMaternityRepository repository;

	/**
	 * This method is used to fetch all the emplvmaternity entries from the database.
	 * @return List<EmpLvMaternityEntity> This returns the list of emplvmaternity entities fetched from the database.
	 */
	public List<EmpLvMaternityEntity> getAllEmpLvMaternitys()
	{
		List<EmpLvMaternityEntity> result = (List<EmpLvMaternityEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (EmpLvMaternityEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<EmpLvMaternityEntity>();
		}
	}
	 ...
	 * @return EmpLvMaternityEntity This returns the EmpLvMaternityEntity object for the requested emplvadoption id.
	 */
	public EmpLvMaternityEntity getEmpLvMaternityById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvMaternityEntity> emplvadoption = repository.findById(id);
	...
	/**
	 * This method is used to update all the columns of the EmpLvMaternity entry in the database.
	 * @param entity This attribute is the EmpLvMaternityEntity object with pre-populated data for all columns.
	 * @return String This returns updated EmpLvMaternityEntity object.
	 */
	public EmpLvMaternityEntity createOrUpdateEmpLvMaternity(EmpLvMaternityEntity entity)
	{
		...
		{
			Optional<EmpLvMaternityEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				EmpLvMaternityEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular emplvmaternity entity.
	 */
	...
	/**
	 * This method is used to delete the emplvmaternity entry from the database by its id.
	 * @param id This attribute is the id of the emplvmaternity entry to be deleted.
	 * @return void
	 */
	public void deleteEmpLvMaternityById(Long id) throws RecordNotFoundException
	{
		Optional<EmpLvMaternityEntity> employee = repository.findById(id);
		
		if(employee.isPresent()) 
		{
			repository.deleteById(id);
		} else {
			throw new RecordNotFoundException("No emplvmaternity record exist for given id");
		}
	} 
}
