...
import com.vetan.repository.LvAdoptionRepository;
...
@Service
public class LvAdoptionService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvAdoptionRepository repository;

	/**
	 * This method is used to fetch all the lvadoption entries from the database.
	 * @return List<LvAdoptionEntity> This returns the list of lvadoption entities fetched from the database.
	 */
	public List<LvAdoptionEntity> getAllLvAdoptions()
	{
		List<LvAdoptionEntity> result = (List<LvAdoptionEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvAdoptionEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvAdoptionEntity>();
		}
	}

	/**
	 * This method is used to get a single lvadoption entry from the database based on its id.
	 * @param id This attribute is the id of the lvadoption entry.
	 * @return LvAdoptionEntity This returns the LvAdoptionEntity object for the requested lvadoption id.
	 */
	public LvAdoptionEntity getLvAdoptionById(Long id) throws RecordNotFoundException
	{
		Optional<LvAdoptionEntity> lvadoption = repository.findById(id);
		
		if(lvadoption.isPresent()) {
			return lvadoption.get();
		} else {
			throw new RecordNotFoundException("No lvadoption record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvAdoption entry in the database.
	 * @param entity This attribute is the LvAdoptionEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvAdoptionEntity object.
	 */
	public LvAdoptionEntity createOrUpdateLvAdoption(LvAdoptionEntity entity)
	{
		...
		{
			Optional<LvAdoptionEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvAdoptionEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvadoption entity.
	 */
	...
	/**
	 * This method is used to delete the lvadoption entry from the database by its id.
	 * @param id This attribute is the id of the lvadoption entry to be deleted.
	 * @return void
	 */
	public void deleteLvAdoptionById(Long id) throws RecordNotFoundException
	{
		Optional<LvAdoptionEntity> employee = repository.findById(id);
