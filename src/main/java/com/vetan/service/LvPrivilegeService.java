...
import com.vetan.repository.LvPrivilegeRepository;
...
@Service
public class LvPrivilegeService {

	@Value("${spring.project.directory}")
	private String targetPath;

	@Autowired
	LvPrivilegeRepository repository;

	/**
	 * This method is used to fetch all the lvprivilege entries from the database.
	 * @return List<LvPrivilegeEntity> This returns the list of lvprivilege entities fetched from the database.
	 */
	public List<LvPrivilegeEntity> getAllLvPrivileges()
	{
		List<LvPrivilegeEntity> result = (List<LvPrivilegeEntity>) repository.findAll();
		
		if(result.size() > 0) {

			for (LvPrivilegeEntity entity:
				 result) {
				retriveAttachmentsAndStore(entity.getAttach());
			}
			return result;
		} else {
			return new ArrayList<LvPrivilegeEntity>();
		}
	}

	/**
	 * This method is used to get a single lvprivilege entry from the database based on its id.
	 * @param id This attribute is the id of the lvprivilege entry.
	 * @return LvPrivilegeEntity This returns the LvPrivilegeEntity object for the requested lvprivilege id.
	 */
	public LvPrivilegeEntity getLvPrivilegeById(Long id) throws RecordNotFoundException
	{
		Optional<LvPrivilegeEntity> lvprivilege = repository.findById(id);
		
		if(lvprivilege.isPresent()) {
			return lvprivilege.get();
		} else {
			throw new RecordNotFoundException("No lvprivilege record exist for given id");
		}
	}

	/**
	 * This method is used to update all the columns of the LvPrivilege entry in the database.
	 * @param entity This attribute is the LvPrivilegeEntity object with pre-populated data for all columns.
	 * @return String This returns updated LvPrivilegeEntity object.
	 */
	public LvPrivilegeEntity createOrUpdateLvPrivilege(LvPrivilegeEntity entity)
	{
		...
		{
			Optional<LvPrivilegeEntity> employee = repository.findById(entity.getId());
			
			if(employee.isPresent()) 
			{
				LvPrivilegeEntity newEntity = employee.get();
	 ...
	 * @return byte[] This returns the byte array of all the attachments for a particular lvprivilege entity.
	 */
	...
	/**
	 * This method is used to delete the lvprivilege entry from the database by its id.
	 * @param id This attribute is the id of the lvprivilege entry to be deleted.
	 * @return void
	 */
	public void deleteLvPrivilegeById(Long id) throws RecordNotFoundException
	{
		Optional<LvPrivilegeEntity> employee = repository.findById(id);
