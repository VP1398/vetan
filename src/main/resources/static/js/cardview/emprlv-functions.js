...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/emprlv/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('ecode')+' - '+button.data('ename'))
        modal.find('#modal-ecode').val(button.data('ecode'))
        modal.find('#modal-ename').val(button.data('ename'))
        modal.find('#modal-category').val(button.data('category'))
        modal.find('#modal-role').val(button.data('role'))
        modal.find('#modal-dateOfJoining').val(button.data('dateofjoining'))
        modal.find('#modal-age').val(button.data('age'))
        modal.find('#modal-location').val(button.data('location'))
        modal.find('#modal-department').val(button.data('department'))
        modal.find('#modal-bcode').val(button.data('bcode'))
        modal.find('#modal-gender').val(button.data('gender'))
         modal.find('#modal-manager').val(button.data('manager'))
        modal.find('#modal-cityType').val(button.data('citytype'))
...
function updateAndSubmit(){
    ...
    document.getElementById("emprlvForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/emprlv/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
