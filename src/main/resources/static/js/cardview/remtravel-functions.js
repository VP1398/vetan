...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remtravel/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('startdatetime')+' - '+button.data('enddatetime'))
        modal.find('#modal-startDateTime').val(button.data('startdateTime'))
        modal.find('#modal-endDateTime').val(button.data('enddatetime'))
        modal.find('#modal-noOfDays').val(button.data('nofDays'))
        modal.find('#modal-expenseDescription').val(button.data('expensedescription'))
        modal.find('#modal-expenseDetails').val(button.data('expensedetails'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remtravelForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remtravel/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
