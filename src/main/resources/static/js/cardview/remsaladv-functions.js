...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remsaladv/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('advancetype')+' - '+button.data('festivalname'))
        modal.find('#modal-advanceType').val(button.data('advancetype'))
        modal.find('#modal-salaryAdvReason').val(button.data('salaryadvreason'))
        modal.find('#modal-entitledAmount').val(button.data('entitledamount'))
        modal.find('#modal-requiredAmount').val(button.data('requiredamount'))
        modal.find('#modal-noofInstalments').val(button.data('noofinstalments'))
        modal.find('#modal-interestRate').val(button.data('interestrate'))
        modal.find('#modal-emi').val(button.data('emi'))
        modal.find('#modal-remarks').val(button.data('remarks'))
        modal.find('#modal-preAdvReason').val(button.data('preadvreason'))
        modal.find('#modal-preDate').val(button.data('predate'))
        modal.find('#modal-amt').val(button.data('amt'))
        modal.find('#modal-repaid').val(button.data('repaid'))
        ...
        $('.amt').on('change', function (event) {
            console.log('inside change listener of .amt');
            styleAmountFields();
        })
...
function updateAndSubmit(){
    ...
    document.getElementById("remsaladvForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remsaladv/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
