...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remloan/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('loantype')+' - '+button.data('productname'))
        modal.find('#modal-loanType').val(button.data('loantype'))
        modal.find('#modal-productName').val(button.data('productname'))
        modal.find('#modal-productType').val(button.data('producttype'))
        ...
        modal.find('#modal-estimateValue').val(button.data('estimatevalue'))
        modal.find('#modal-totestValue').val(button.data('totestvalue'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remloanForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remloan/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
