...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remlcon/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('localconveyance')+' - '+button.data('purpose'))
        modal.find('#modal-localConveyance').val(button.data('localconveyance'))
        modal.find('#modal-purpose').val(button.data('purpose'))
        modal.find('#modal-date').val(button.data('date'))
        modal.find('#modal-startTime').val(button.data('starttime'))
        modal.find('#modal-endTime').val(button.data('endtime'))
        modal.find('#modal-from').val(button.data('from'))
        modal.find('#modal-destination').val(button.data('destination'))
        modal.find('#modal-vehicleType').val(button.data('vehicletype'))
        modal.find('#modal-transportMode').val(button.data('transportmode'))
        modal.find('#modal-kmsNo').val(button.data('kmsno'))
        modal.find('#modal-rate').val(button.data('rate'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remlconForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remlcon/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
