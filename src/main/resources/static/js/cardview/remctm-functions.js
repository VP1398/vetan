...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remctm/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('Claimdate')+' - '+button.data('empcode'))
        modal.find('#modal-ClaimDate').val(button.data('Claimdate'))
        modal.find('#modal-empCode').val(button.data('empcode'))
        modal.find('#modal-name').val(button.data('name'))
        ...
        modal.find('#modal-invoiceNo').val(button.data('invoiceNo'))
        ...
        modal.find('#modal-mremarks').val(button.data('mremarks'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remctmForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remctm/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
