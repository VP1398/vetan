...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remscholar/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('scholarshiptype')+' - '+button.data('childname'))
        modal.find('#modal-scholarshipType').val(button.data('scholarshiptype'))
        modal.find('#modal-childName').val(button.data('childname'))
        modal.find('#modal-relationship').val(button.data('relationship'))
        modal.find('#modal-financialYear').val(button.data('financialyear'))
        modal.find('#modal-collegeName').val(button.data('collegename'))
        modal.find('#modal-applicationType').val(button.data('applicationtype'))
        modal.find('#modal-passingYear').val(button.data('passingyear'))
        modal.find('#modal-percentage').val(button.data('percentage'))
        modal.find('#modal-scholarshipAmount').val(button.data('scholarshipamount'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remscholarForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remscholar/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
