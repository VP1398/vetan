...
function initListeners(){
    ...
    document.getElementById("inputGroupFile03").onclick = function(){
        ...
        $.ajax({
            url: '/CardView/remloannsc/uploadFile',
            type: 'POST',
            processData: false,
            contentType: false,
            dataType : 'json',
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#exampleModalLabel').text(button.data('loantype')+' - '+button.data('nscnumber'))
        modal.find('#modal-loanType').val(button.data('loantype'))
        modal.find('#modal-nscNumber').val(button.data('nscnumber'))
        modal.find('#modal-noOfCertificates').val(button.data('noofcertificates'))
        modal.find('#modal-nscPurchaseDate').val(button.data('nscpurchasedate'))
        modal.find('#modal-nscValue').val(button.data('nscvalue'))
        modal.find('#modal-nscMaturityDate').val(button.data('nscMaturitydate'))
        modal.find('#modal-requestedAmount').val(button.data('requestedamount'))
...
function updateAndSubmit(){
    ...
    document.getElementById("remloannscForm").submit();
}
...
function removeAttachment(id){
    ...
    $.ajax({
        url: '/CardView/remloannsc/deleteFile',
        type: 'POST',
        processData: false,
        contentType: false,
        dataType : 'json',
